...
router.get('/savepin/:item',async(req,res)=>{
    ...
    try{
        const pin=await videomodel.findById(id).exec();
        res.send(pin);
...
router.get('/',async(req,res)=>{
    const videos=await videomodel.find().sort('-created_At').exec();
    res.send(videos)
})
router.get('/category/:category',async(req,res)=>{
    ...
    if(vote==='Date')
    {
        try{
            ...
            const data=await videomodel.find().sort('-created_At').exec();
            res.send(data);
        }
        catch(err)
        {
    ...
    {
    const filterdata=await videomodel.find().sort(`-${vote.length}`).exec();
