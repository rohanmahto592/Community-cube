...
const Videopin = ({user,pin,check}) => {
  ...
  return (
    ...
    {check2===true && <Alertmessage open={open} setcheck={setcheck2} setOpen={setOpen} type="warning" message={`Already upvoted the ${pin.created_by.userName.slice(0,-11)}'s Video post`}/>}
        ...
        avatar={
          <Avatar
            onClick={() => navigate(`/user-profile/${pin?.created_by?.id}`)}
     ...
     {pin.video &&(
       ...
       {/* {pin?.pdf &&(
         <a href={pin?.pdf} target='_blank' download="pdf">
         <button
       ...
       {pin?.category && <Chip  style={{fontFamily:'comic sans ms',color:'slateblue',fontWeight:'bold',marginBottom:'6px'}} label={pin.category}  />}
          ...
          <Chip
            ...
            onClick={Likes} 
            avatar={<Avatar alt="Natacha" src={like} />}
            label={pin?.likes?.length > 0 ? pin.likes.length : 0}
