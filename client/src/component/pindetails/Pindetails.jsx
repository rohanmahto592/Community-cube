...
const Pindetails = ({ user }) => {
  ...
  useEffect(() => {
    const id = pinId;
    axios.get(`http://localhost:3001/posts/doubt/${id}`).then((response) => {
  ...
  const addComment=async()=>{
    ...
    const id = pinId;
    axios.put(`http://localhost:3001/posts/doubt/${id}`,data).then((response)=>{
      setcheck(true)
      setOpen(true)
    })
  ...
  return (
    ...
    {check && <Alertmessage open={open} setOpen={setOpen} message="commented successfully" type='info'/>}
      {doubt && (
        <div
          className="flex xl:flex-column mt-8 flex-col m-auto bg-white"
          style={{  maxWidth: "1500px" }}
        >
         {
             doubt?.images?.length>0 &&(
                <div className="flex bg-gray-800 justify-center items-center md:items-start flex-initial">
                <Imageslider images={doubt.images}/>
            </div>
        ...
        <Link to={`/user-profile/${doubt?.created_by}`} className="flex gap-2 mt-2 items-center bg-gray-800 rounded-lg ">
              <img src={doubt?.created_by_image} className="w-11 h-11 rounded-full" alt="user-profile" />
             
            </Link>
            <Chip  style={{fontFamily:'comic sans ms',color:'slateblue',fontWeight:'bold',marginBottom:'1vh',marginTop:'1vh',}} label={`Doubt posted ${moment(new Date(doubt.Date)).fromNow()}`} />
